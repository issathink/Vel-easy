...
public class VelWidgetProvider extends AppWidgetProvider {
    ...
    public void onUpdate(Context context, AppWidgetManager appWidgetManager, int[] appWidgetIds) {
        ...
        if(favNames.size() == 0 || favNumbers.size()==0){
            views.setTextViewText(R.id.fav1, "Pas de favoris");
            views.setTextViewText(R.id.fav2, "Pas de favoris");
            for(int i=0;i<appWidgetIds.length;i++)
                appWidgetManager.updateAppWidget(appWidgetIds[i], views);
            return;
        }
        if(favNames.size() == 1 || favNumbers.size()==1) {
            info_pos = new String[1];
            info_num = new String[1];
            views.setTextViewText(R.id.nbVelib2,"0");
            views.setTextViewText(R.id.nbPlace2, "0");
            views.setTextViewText(R.id.fav2, "Station favorite");
            value = 1;
        }
        else{
       ...
       for(int i=0;i<info_pos.length;i++){
        ...
        String Urlloc2="";
       if(value!=1)
           Urlloc2= URL_principale+info_num[1];
